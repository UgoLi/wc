...
void WebCrawler::init (
	...
	// 若切换工作目录至下载路径失败
	if (chdir (m_cfg.m_downloadPath.c_str ()) == -1)
		// 记录一般错误日志
...
bool WebCrawler::startJob (void) {
	...
	Socket* socket = new Socket (m_urlQueues.popDnsUrl ());
	...
	epoll_event event = {EPOLLIN | EPOLLET, socket};
	// 增加需要被关注的文件描述符，若失败
	if (! m_multiIo.add (socket->sockfd (), event)) {
		// 销毁套接字对象
		delete socket;
		// 解锁互斥锁
		pthread_mutex_unlock (&m_mutex);
...
void WebCrawler::initSeedUrls (void) {
	// 若配置器中的种子统一资源定位符为空
	if (m_cfg.m_seedUrls.empty ())
		// 记录一般错误日志
		m_log.printf (Log::LEVEL_ERR, __FILE__, __LINE__,
			"没种子咋干活嘞");

	// 拆分种子统一资源定位符字符串，以逗号为分隔符，不限拆分次数
	vector<string> seedUrls = StrKit::split (m_cfg.m_seedUrls, ",", 0);

	// 依次处理每个种子统一资源定位符
	for (vector<string>::iterator it = seedUrls.begin ();
		it != seedUrls.end (); ++it) {
		// 若规格化失败
		if (! RawUrl::normalized (*it))
			// 记录一般错误日志
			m_log.printf (Log::LEVEL_ERR, __FILE__, __LINE__,
				"种子统一资源定位符(%s)规格化失败", it->c_str ());

		// 若原始统一资源定位符队列满
		if (m_urlQueues.fullRawUrl ())
			// 记录一般错误日志
			m_log.printf (Log::LEVEL_ERR, __FILE__, __LINE__,
				"种子太多或者队列太小");

...
void Configurator::load (
	...
	for (int lineNo = 0; getline (ifs, line); ++lineNo) {
		...
		if (strv.size () == 2) {
			...
			// 若键为"MAX_FAILURES"
			if (! strcasecmp (strv[0].c_str (), "MAX_FAILURES"))
				// 则值为"最大失败次数"
				m_maxFailures = atoi (strv[1].c_str ());
			// 否则
			else
			// 若键为"MAX_RAW_URLS"
			...
			// 若键为"MAX_DOMAINS"
			if (! strcasecmp (strv[0].c_str (), "MAX_DOMAINS"))
				// 则值为"域名映射表最大容量"
				m_maxDomains = atoi (strv[1].c_str ());
			// 否则
			else
			// 若键为"RES_TIMEOUT"
			if (! strcasecmp (strv[0].c_str (), "RES_TIMEOUT"))
				// 则值为"响应超时"
				m_resTimeout = atoi (strv[1].c_str ());
			// 否则
			else
			// 若键为"STAT_INTERVAL"
			...
			// 若键为"SEED_URLS"
			if (! strcasecmp (strv[0].c_str (), "SEED_URLS"))
				// 则值为"种子统一资源定位符"
				m_seedUrls = strv[1];
			// 否则
			else
			...
			// 若键为"DOWNLOAD_PATH"
			if (! strcasecmp (strv[0].c_str (), "DOWNLOAD_PATH"))
				// 则值为"下载路径"
				m_downloadPath = strv[1];
			// 否则
			else
			// 若键为"PLUGINS_PATH"

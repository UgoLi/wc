...
bool RawUrl::valid (
	...
	// 若不存在非法字符，则返回true，否则返回false
	return strUrl.find_first_not_of (
...
DnsUrl::DnsUrl (
	...
	// 否则
		...
		// 资源路径最后一个'/'之前为资源目录
		if ((pos = m_path.find_last_of ('/')) != string::npos)
...
bool DnsUrl::attachDomain (
	...
	// 若待加域名统一资源定位符首字符不是'/'
	if (strUrl[0] != '/') {
		// 将资源目录插到待加域名统一资源定位符字符串首字符之前
		strUrl.insert (0, m_dir);
		// 将'/'插到待加域名统一资源定位符字符串首字符之前
		strUrl.insert (0, "/");
	}

...
// 字符串拆分
// 返回被拆分出的子串向量
// 以sep中的字符作为分隔符，对str字符串进行拆分，
// 并对每个被拆分出的子串做修剪，拆分次数不超过max，
// 除非该参数的值为0
vector<string> StrKit::split (
	string const& str,          // [输入] 待拆分字符串
	string const& sep,          // [输入] 分隔符字符串
	int           max /* = 0 */ // [输入] 最多拆分次数
	) {
	// 存放拆分结果的子串向量
	vector<string> strv;
	...
	if (buf) {
		...
		//   sep : " ,.:;()$"
		//   max : 3
		//   buf : The quick,brown.fox:jumps;over(the)lazy$dog/0
		//         t  /t    /t    /t  /t    /t   /t  /t   /t
		...
		for (char* token = strtok (buf, sep.c_str ());
			token; token = strtok (NULL, sep.c_str ())) {
			// 被拆分出的子串,strtok返回不包含分割标记的第一个字符串首地址
            //往后遍历第一个参数设置为NULL 
			string part = token;
			// 经修剪后压入存放拆分结果的子串向量
			strv.push_back (trim (part));

			// 若拆分次数限制已到(若max参数取缺省值0，则此条件永远不
			// 可能满足，即不限制拆分次数)，且本次所拆并非最后一个子串
			if (! --max && (token += strlen (token)) - buf <
				(int)str.size ()) {
				// 将待拆分字符串的其余部分一次
				// 性压入存放拆分结果的子串向量
...
int main (void) {
	...
	vector<string> strv = StrKit::split (str, "=", 3);
